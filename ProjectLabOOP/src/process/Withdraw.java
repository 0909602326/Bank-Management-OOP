...
public class Withdraw extends JPanel {
	 ...
	 * Create the panel.
	 */
	public Withdraw(JPanel ex_panel, JFrame frame, Customer ctm) {
		...
		panel_50k.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				WithdrawMoney("50,000", ctm, 50000);
			}
		...
		panel_100k.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblCheck.setVisible(true);
				WithdrawMoney("100,000", ctm, 100000);
			}
		...
		panel_200k.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblCheck.setVisible(true);
				WithdrawMoney("200,000", ctm, 200000);
			}
		...
		panel_500k.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblCheck.setVisible(true);
				WithdrawMoney("500,000", ctm, 500000);
			}
		...
		panel_1tr.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblCheck.setVisible(true);
				WithdrawMoney("1,000,000", ctm, 1000000);
			}
		...
		panel_2tr.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				lblCheck.setVisible(true);
				WithdrawMoney("2,000,000", ctm, 2000000);
			}
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (m != null) {
					if (m.equalsIgnoreCase(""))
						...
						WithdrawMoney(m,ctm,Double.parseDouble(m));
					}
				}
			}
			@Override
		...
		lblNewLabel_1.setIcon(new ImageIcon(Withdraw.class.getResource("/images/Finance-Cash-Receiving-icon.png")));
	...
	public void WithdrawMoney(String t, Customer ctm, double money) {
		boolean flag = true;
		int click = JOptionPane.showConfirmDialog(null, "Are you sure you want to withdraw " + t + " VND?", "Message",
				JOptionPane.YES_NO_OPTION);
		if (click == JOptionPane.YES_OPTION) {
			for (Customer cs : list.getList()) {
				if (cs.getPIN().equalsIgnoreCase(ctm.getPIN())) {
					ctm.Withdraw(money);
					flag = cs.Withdraw(money);
					break;
				}
			}
			if (flag == true) {
				JOptionPane.showMessageDialog(null, "Successfully Withdraw", "Message", 1);
				String month = String.valueOf(date.now().getMonth()).substring(0, 2);
				String time = month + "-" + date.now().getDayOfMonth() + "-" + date.now().getYear();
				Exchange ex = new Exchange(time, "Withdraw", money, "0");
				exh.addEvent(ex);
				try {
				...
				JOptionPane.showMessageDialog(null, "Please check your balance", "Alert", 2);
			}
		}
	}
}
